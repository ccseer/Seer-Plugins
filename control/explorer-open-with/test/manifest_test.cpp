...
{
    return value && value->kind == JsonValue::Kind::Array && value->arrayValue.size() == 1
...
{
    if (argc != 2) {
        std::cerr << "usage: openwith_manifest_test <staging-root>\n";
        return 2;
    }

    try {
        const std::filesystem::path stagingRoot = std::filesystem::weakly_canonical(argv[1]);
        ...
        check(command && command->kind == JsonValue::Kind::String
                  && command->stringValue == "shellopenwith.exe",
        ...
        if (command && command->kind == JsonValue::Kind::String) {
            ...
            check(!executableName.is_absolute() && !executableName.has_parent_path(),
                  "command is package-relative");

            const auto executablePath = std::filesystem::weakly_canonical(stagingRoot / executableName);
